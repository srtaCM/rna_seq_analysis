...
# procdata_dir was undefined/missing params$procdata_dir
...
##CLE and TRAP 
sample_info$extraction <- "CLE"  # Default
sample_info$extraction[rownames(sample_info) %in% 
  c("H-107", "H-108", "H-109", "H-110", "H-111",    # DIT-TRAP  
    "IM-206", "IM-207", "IM-208", "IM-209", "IM-210",
    "UM-204", "UM-208", "UM-213", "UM-214", "UM-215")] <- "TRAP"
# Factors
sample_info$line <- factor(sample_info$line, levels = c("bulk", "35SpPETEp", "DMR6pPETEp"))
sample_info$extraction <- factor(sample_info$extraction)
```
...
print("Match check:", all(colnames(counts_trap) == rownames(col_data_trap)))
...
h_summary <- sample_info_trap %>%
  ...
  mutate(
    baseline = mean_h[line == "35SpPETEp"],  # Store 35S value
    H_index = mean_h / baseline
  ) %>%
  select(-baseline)  # Clean up
print("🚀 HAUSTORIA H-INDEX (TRAP):")
print(h_summary)
...
# Plot
library(ggplot2)
ggplot(sample_info_trap, aes(line, h_index, fill = line)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.7) +
  labs(y = "H-index (mean rlog haustoria genes)",
       title = "DMR6p;PETEp vs 35Sp;PETEp TRAP") +
  theme_minimal()
```
