...
sample_info <- data.frame(
  Sample = c("IT-107", "IT-108", "IT-109", "IT-110", 
             "H-107", "H-108", "H-109", "H-110", "H-111",
             "UT-106", "UT-108", "UT-114", 
             "IT-206", "IT-207", "IT-208", "IT-209",
             "IM-206", "IM-207", "IM-208", "IM-209", "IM-210",
...
%>%
  # Now use $Sample (column) instead of rownames
  mutate(
    prefix = substr(Sample, 1, 3),    # "IT-", "H-1", "UT-", "IM-", "UM-"
    
    # DMR6p;PETEp (414C4): H-107..111 (DIT), IT-107..110 (DIC) 
    # 35Sp;PETEp (419F2): all others
    line = case_when(
      grepl("^H-1", Sample) ~ "DMR6pPETEp",  # H-107 to H-111 = DIT (TRAP, low yield)
      TRUE ~ "35SpPETEp"                     # Everything else
    ),
    
    condition = case_when(
      grepl("^(UT|UM)", Sample) ~ "mock",
      TRUE ~ "infected"
    ),
    
    # TRAP (first batch, low DMR6 yields) vs CLE (second batch)
    extraction = case_when(
      Sample %in% c("IT-107", "IT-108", "IT-109", "IT-110",
                    "H-107", "H-108", "H-109", "H-110", "H-111",
                    "UT-106", "UT-108", "UT-114") ~ "TRAP",
      TRUE ~ "CLE"
    )
  ) %>%
  mutate(
    line = factor(line, levels = c("35SpPETEp", "DMR6pPETEp")),
    condition = factor(condition, levels = c("mock", "infected")),
...
print("✅ Meta")
print(table(sample_info$line, sample_info$extraction))
print(table(sample_info$line, sample_info$condition))
...
```{r}
# Decode line, condition, extraction from the prefixes you defined earlier
sample_info <- sample_info %>%
  mutate(
    prefix = substr(Sample, 1, 2),   # "IT", "H-", "UT", "IM", "UM"

    # 419F2 (35S::GFP1-9 + PETE::GFP10-11) vs 414C4 (DMR6::GFP1-9 + PETE::GFP10-11)
    # Here we follow your earlier mapping: 
    # DIC = IT, DIT = H, DMC = UT, PIC = IT, PIT = IM, PMC = UT, PMT = UM
    # →  IT / IM / UT / UM are 35Sp;PETEp, H is DMR6p;PETEp
    line = case_when(
      grepl("^H-",  Sample) ~ "DMR6pPETEp",  # H-107..H-111 = DMR6 line (haustoria TRAP)
      TRUE                   ~ "35SpPETEp"   # Everything else = 35S line
    ),
    ...
    # TRAP vs CLE: first block of each genotype are TRAP (IT, H, UT),
    # second block (IM, UM, UT-2xx) are CLE
    extraction = case_when(
      grepl("^(IT-|H-|UT-1)", Sample) ~ "TRAP",
      TRUE                            ~ "CLE"
    )
  ) %>%
  mutate(
...
# 2. TRAP-only DESeq2 object (clean haustoria signal)
keep_trap   <- sample_info$extraction == "TRAP"
counts_trap <- as.matrix(read.count[, keep_trap])

dds_trap <- DESeqDataSetFromMatrix(
  countData = round(counts_trap),
  colData   = sample_info[keep_trap, ],
  design    = ~ line + condition
)

dds_trap$line <- relevel(dds_trap$line, ref = "35SpPETEp")
dds_trap <- DESeq(dds_trap)
rld_trap <- rlog(dds_trap)


# 3. Haustoria marker set (can expand later) [web:54][web:56][web:62]
haustoria_genes <- c(
  "AT5G24530",  # DMR6
  "AT2G14610",  # PR1
  "AT3G04720",  # PR2
...
cat("Using", length(haustoria_genes), "haustoria markers present in this dataset\n")

h_mat   <- assay(rld_trap)[haustoria_genes, , drop = FALSE]
h_index <- colMeans(h_mat)

sample_info_trap <- sample_info[keep_trap, ]
sample_info_trap$h_index <- h_index

# 4. Summarize to a line-level H-index (your haustoria metric)
h_summary <- sample_info_trap %>%
  group_by(line, condition) %>%
  summarise(
    mean_h = mean(h_index),
  ...
  tidyr::pivot_wider(names_from = line, values_from = c(mean_h, se_h, n)) %>%
  mutate(
    H_index = mean_h_DMR6pPETEp / mean_h_35SpPETEp
  )

print(h_summary)

## Quick H-index plot
ggplot(sample_info_trap, aes(line, h_index, fill = condition)) +
  geom_boxplot() +
  geom_jitter(width = 0.15, alpha = 0.6) +
  theme_minimal() +
  labs(y = "H-index (mean rlog haustoria markers)",
       title = "Haustoria enrichment in DMR6p;PETEp vs 35Sp;PETEp TRAP")

```


```{r}
sample_info$condition <- ifelse(grepl("^[IU][MT]-", sample_info$Sample), "mock", "infected")
sample_info$extraction <- ifelse(grepl("-1", sample_info$Sample), "TRAP", "CLE")  # -1xx=TRAP, -2xx=CLE

sample_info$line <- factor(sample_info$line, levels=c("35SpPETEp", "DMR6pPETEp"))
sample_info$condition <- factor(sample_info$condition, levels=c("mock", "infected"))

# Verify
stopifnot(all(rownames(sample_info) %in% colnames(your_counts_matrix)))
table(sample_info$line, sample_info$extraction)  # Should show DMR6 TRAP low-yield
...
```{r}
# TRAP-only for cleanest haustoria signal
dds_trap <- DESeqDataSetFromMatrix(
  countData = counts_matrix[, sample_info$extraction == "TRAP"],
  colData = sample_info[sample_info$extraction == "TRAP", ],
  design = ~ line + condition
)
...
# H-index using DMR6 markers
haustoria_genes <- c("AT1G75040", "AT2G14610", ...)  # From before
h_index <- colMeans(assay(rld_trap)[haustoria_genes, ])

sample_info_trap$h_index <- h_index
h_summary <- sample_info_trap %>%
  group_by(line, condition) %>%
  summarise(mean_h = mean(h_index), se = sd(h_index)/sqrt(n()), .groups="drop") %>%
  pivot_wider(names_from=line, values_from=c(mean_h, se)) %>%
  mutate(H_index = mean_h_DMR6pPETEp / mean_h_35SpPETEp)

print(h_summary)  # DMR6p;PETEp infected >> 35Sp;PETEp = haustoria enrichment!
