...
if (!dir.exists(procdata_dir)) dir.create(procdata_dir, recursive = TRUE)
...
## 4. Load GFP Mapped Counts
```{r gfp-reads}
## LOAD BOWTIE2 MAPPING SUMMARIES
gfp1_9_summary <- read.table(
  "../Bowtie/GFP1_9_mapping_summary.tsv",
  header = TRUE, sep = "\t", stringsAsFactors = FALSE
)
gfp10_11_summary <- read.table(
  "../Bowtie/GFP10_11_mapping_summary.tsv",
  header = TRUE, sep = "\t", stringsAsFactors = FALSE
)
...
## 6. Rename featcounts and GFP summaries
```{r new-names-for-everything}
## Read counts (all data)
read.count_basenames <- colnames(read.count)
read.count_basenames <- str_remove(read.count_basenames, "\\.bam$")
read.count_basenames <- str_remove(read.count_basenames, "Aligned\\.sortedByCoord\\.out$")
read.count_basenames <- str_remove(read.count_basenames, "out\\.bam$")

## GFP Mapped
gfp1_9_basenames <- gfp1_9_summary$sample_id
gfp10_11_basenames <- gfp10_11_summary$sample_id

# Verify match
all(read.count_basenames == gfp1_9_basenames)  # TRUE, breaks often
# Find exact mismatches
#setdiff(read.count_basenames, gfp1_9_basenames)
#which(read.count_basenames != gfp1_9_basenames)
...
#read_counts_df <- as.data.frame(read.count)
#read_counts_df <- tibble::rownames_to_column(read_counts_df, "GeneID")
```
...
```{r complete metadata}
## META DATA
sample_info <- data.frame(row.names = colnames(read.count))  

## set bulk (CLE samples) or TRAP 
sample_info$line <- "bulk"  
bulk_samples <- c("IT-107", "IT-108", "IT-109", "IT-110", "IT-206", "IT-207", "IT-208", "IT-209", "UT-106", "UT-108", "UT-114","UT-204", "UT-208", "UT-213", "UT-214")
sample_info$line[rownames(sample_info) %in% bulk_samples] <- "bulk"

sample_info$line[rownames(sample_info) %in% 
  c("H-107", "H-108", "H-109", "H-110", "H-111")] <- "DMR6pPETEp"
sample_info$line[rownames(sample_info) %in% 
  c("IM-206", "IM-207", "IM-208", "IM-209", "IM-210")] <- "35SpPETEp" 
sample_info$line[rownames(sample_info) %in% 
  c("UM-204", "UM-208", "UM-213", "UM-214", "UM-215")] <- "35SpPETEp"  

sample_info$extraction <- "CLE"
sample_info$extraction[rownames(sample_info) %in% 
  c("H-107", "H-108", "H-109", "H-110", "H-111",
    "IM-206", "IM-207", "IM-208", "IM-209", "IM-210",
    "UM-204", "UM-208", "UM-213", "UM-214", "UM-215")] <- "TRAP"


## SET TREATMENT: IM H UM IT UT
sample_info$trt <- "TRT"
sample_info$trt[rownames(sample_info) %in% 
  c("H-107", "H-108", "H-109", "H-110", "H-111")] <- "H"
sample_info$trt[rownames(sample_info) %in% 
  c("IM-206", "IM-207", "IM-208", "IM-209", "IM-210")] <- "IM" 
sample_info$trt[rownames(sample_info) %in% 
  c("UM-204", "UM-208", "UM-213", "UM-214", "UM-215")] <- "UM" 
sample_info$trt[rownames(sample_info) %in% 
  c("IT-107", "IT-108", "IT-109", "IT-110", "IT-206", "IT-207", "IT-208", "IT-209")] <- "IT" 
sample_info$trt[rownames(sample_info) %in% 
  c("UT-106", "UT-108", "UT-114", "UT-204", "UT-208", "UT-213", "UT-214")] <- "UT" 
...
print(table(sample_info$trt, sample_info$extraction))
```

## 8. Extraction of H v IM v UM From Summaries
```{r}
...
# Extract from GFP1-9
h_cols1 <- grep(h_pattern, gfp1_9_summary$sample_id, value = TRUE)
im_cols1 <- grep(im_pattern, gfp1_9_summary$sample_id, value = TRUE)
um_cols1 <- grep(um_pattern, gfp1_9_summary$sample_id, value = TRUE)

gfp1_9_h_im_um <- gfp1_9_summary[gfp1_9_summary$sample_id %in% c(h_cols1, im_cols1, um_cols1), ]

# Extract from GFP10-11
h_cols2 <- grep(h_pattern, gfp10_11_summary$sample_id, value = TRUE)
im_cols2 <- grep(im_pattern, gfp10_11_summary$sample_id, value = TRUE)
um_cols2 <- grep(um_pattern, gfp10_11_summary$sample_id, value = TRUE)

gfp10_11_h_im_um <- gfp10_11_summary[gfp10_11_summary$sample_id %in% c(h_cols2, im_cols2, um_cols2), ]

cat("GFP1-9 subset:\n")
cat("H:", length(h_cols1), "| IM:", length(im_cols1), "| UM:", length(um_cols1), "\n")
cat("Dims:", nrow(gfp1_9_h_im_um), "x", ncol(gfp1_9_h_im_um), "\n\n")

cat("GFP10-11 subset:\n")
cat("H:", length(h_cols2), "| IM:", length(im_cols2), "| UM:", length(um_cols2), "\n")
cat("Dims:", nrow(gfp10_11_h_im_um), "x", ncol(gfp10_11_h_im_um), "\n")

# Export separately

# View high-GFP H/IM samples for QC
print("High GFP1-9 H/IM (>10%):")
print(gfp1_9_h_im_um[grep("H-|IM-", gfp1_9_h_im_um$sample_id), "mapped_pct"])

```

## 9. What Fraction of Reads Align to the GFP1-9 Construct Rather Than Transcriptome?
```{r}
## Calculate GFP Reads 
gfp1_9_percent <- gfp1_9_h_im_um %>%
  mutate(mapped_pct = GFP1_9_mapped_reads / total_reads * 100)
gfp10_11_percent <- gfp10_11_h_im_um %>%
  mutate(mapped_pct = GFP10_11_mapped_reads / total_reads * 100)


## Plot GFP1-9
ggplot(gfp1_9_percent, aes(x = sample_id, y = mapped_pct, fill = sample_id)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylab("Percent of reads mapping to GFP1-9") +
  ggtitle("GFP1-9 Mapping Summary") +
  theme(legend.position = "none")


## Plot GFP10-11
ggplot(gfp10_11_percent, aes(x = sample_id, y = mapped_pct, fill = sample_id)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylab("Percent of reads mapping to GFP10-11") +
  ggtitle("GFP10-11 Mapping Summary") +
  theme(legend.position = "none")


library(patchwork)  # For side-by-side
df_combined <- bind_rows(gfp1_9_percent %>% mutate(Tag = "GFP1-9"), 
                         gfp10_11_percent %>% mutate(Tag = "GFP10-11"))
ggplot(df_combined, aes(sample_id, mapped_pct, fill = Tag)) + 
  geom_col(position = "dodge") + 
  facet_wrap(~ Tag, scales = "free_x") +  # Or "fixed" for same y
  ylim(0, 1.5)  + # Force common scale
  theme(
...
## 10. Boxplot
```{r}
# Add "condition" column based on sample_id prefix
gfp1_9_h_im_um <- gfp1_9_h_im_um %>%
  mutate(condition = case_when(
    grepl("^H", sample_id) ~ "H",
    grepl("^IM", sample_id) ~ "IM",
    TRUE ~ "other"
  ),
    GFP = "GFP1-19",
    mapped_pct = GFP1_9_mapped_reads / total_reads * 100
  )
...
gfp1_9_h_im_um %>%
  group_by(condition) %>%
  summarise(
    mean_pct = mean(mapped_pct),
    sd_pct = sd(mapped_pct),
    n = n()
  )

ggplot(gfp1_9_h_im_um %>% filter(condition %in% c("H","IM")),
       aes(x = condition, y = mapped_pct, fill = condition)) +
  geom_boxplot() +
  geom_jitter(width = 0.2) +
  ylab("Percent of reads mapping to GFP1-9") +
  ggtitle("H vs IM GFP1-9 Enrichment")
...
gfp10_11_h_im_um <- gfp10_11_h_im_um %>%
  mutate(
    ...
    GFP = "GFP10-11",
    mapped_pct = GFP10_11_mapped_reads / total_reads * 100
  )

# Combine the two datasets
gfp_combined <- bind_rows(
  gfp1_9_h_im_um %>% dplyr::select(sample_id, condition, GFP, mapped_pct),
  gfp10_11_h_im_um %>% dplyr::select(sample_id, condition, GFP, mapped_pct)
)

# Filter to just H and IM for clarity
gfp_combined <- gfp_combined %>% filter(condition %in% c("H","IM", "UM"))

# Plot grouped boxplots with jitter
ggplot(gfp_combined, aes(x = condition, y = mapped_pct, fill = GFP)) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA) +
  geom_jitter(aes(color = GFP), width = 0.2, size = 2, alpha = 0.8) +
  facet_wrap(~GFP, scales = "free_y") +
  ylab("Percent of reads mapping") +
  xlab("Condition") +
  ggtitle("H vs IM: GFP1-9 and GFP10-11 Mapping") +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
```
```{r}
# QC table: medians by condition (H/IM/UM)
gfp_stats <- gfp_combined %>% 
  mutate(condition = case_when(
    grepl("^H-", sample_id) ~ "H",
    grepl("^IM-", sample_id) ~ "IM",
    grepl("^UM-", sample_id) ~ "UM",
    TRUE ~ "Other"
  )) %>%
  ...
  mutate(
    reliable = ifelse(GFP=="GFP10-11" & median_pct < 0.01, "⚠️ Low signal", "✅ OK")
  )

print(gt(gfp_stats))  # Nicely formatted table

# Flag failed reps (GFP10-11 H < 2x UM median)
um_median_10_11 <- median(gfp10_11_h_im_um$mapped_pct[grepl("^UM-", gfp10_11_h_im_um$sample_id)])
failed_h <- gfp10_11_h_im_um %>% 
  filter(grepl("^H-", sample_id), mapped_pct < 2 * um_median_10_11)
print(paste("Failed H reps (GFP10-11 <", round(2*um_median_10_11,4), "%):", failed_h$sample_id))
...
#it_cols <- grep("^IT-", colnames(read.count), value=TRUE)

#im_rows <- gfp10_11_summary$sample_id %in% im_cols
#haust_prop_im <- gfp10_11_summary[im_rows, "mapped_pct"] / 100

#clean_im<- read.count[, im_cols] <- pmax(1, read.count[, im_cols] * (1 - haust_prop_im))

keep_trap <- sample_info$extraction == "TRAP"  # H/IM/UM only
counts_trap <- round(read.count[, keep_trap])
col_data_trap <- sample_info[keep_trap, ]

dds_trap <- DESeqDataSetFromMatrix(counts_trap, col_data_trap, ~ trt)
dds_trap <- DESeq(dds_trap)
vsd <- vst(dds_trap)

# Validate correlation (first 5 matching samples)
n_match <- min(length(im_cols), length(it_cols))



cor_im_it <- cor(t(assay(vsd)[, im_cols[1:n_match]]), 
                 t(assay(vsd)[, it_cols[1:n_match]]), use="complete.obs")


p1<- plotPCA(vsd, intgroup="trt")
print(p1)

print("Available:")
resultsNames(dds)

res_haus <- results(dds, contrast=c("trt", "H", "UM"))
res_haus <- res_haus[order(res_haus$padj), ]

# 3. H-genes (padj < 0.05 + LFC > 2)
h_genes <- rownames(res_haus)[!is.na(res_haus$padj) & 
                              res_haus$padj < 0.05 & 
                              res_haus$log2FoldChange > 2]
print(paste("Haustoria genes:", length(h_genes)))

print(colnames(vsd))  

h_mat <- assay(vsd)[h_genes, ]
h_index <- colMeans(h_mat)
# Match TRAP-only sample_info
sample_info_trap <- sample_info[colnames(vsd), ]  # 15 rows only
h_by_trt <- aggregate(h_index, by=list(sample_info_trap$trt), mean)
colnames(h_by_trt) <- c("trt", "h_index")
print(h_by_trt)

# Visualize
barplot(h_by_trt$h_index, names.arg=h_by_trt$trt, 
        main="Haustoria H-index (TRAP)", ylab="Mean expression")

print(h_by_trt)  # H >> IM > UM
...
# 4. PCA
plotPCA(vsd, intgroup = "trt", nt=3)

# Check lib sizes unchanged
summary(colSums(read.count[, im_cols]))

# Correlate cleaned IM vs IT (expect >0.95)
cor_im_it <- cor(t(assay(vst(dds))[, im_cols]), 
                 t(assay(vst(dds))[, grep("IT-", colnames(dds))]))
