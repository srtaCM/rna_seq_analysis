...
# Sample names (authoritative)
h_samples  <- colnames(trap.count)[h_idx]
im_samples <- colnames(trap.count)[im_idx]

## -----------------------------
## H profile (composition)
## -----------------------------
...
h_comp_vec[is.na(h_comp_vec)] <- 0

## -----------------------------
## GFP fractions (ALIGNED)
## -----------------------------

# H GFP (ordered to match trap.count)
gfp_h <- gfp1_9_percent %>%
  dplyr::filter(sample_id %in% h_samples) %>%
  dplyr::arrange(match(sample_id, h_samples)) %>%
  dplyr::pull(mapped_pct)


## Build named GFP vector aligned to trap.count
gfp_im <- gfp1_9_percent %>%
  filter(sample_id %in% im_samples) %>%
  select(sample_id, mapped_pct) %>%
  deframe() %>%                    # named vector
  .[im_samples]                    # reorder explicitly

stopifnot(
  length(gfp_im) == length(im_idx),
  identical(names(gfp_im), im_samples)
)


# UM GFP (baseline only; ordering irrelevant)
gfp_um <- gfp1_9_percent %>%
  dplyr::filter(str_detect(sample_id, "^UM-")) %>%
  dplyr::pull(mapped_pct)
...
gfp_h_corr  <- pmax(0, gfp_h - um_ref)
gfp_im_corr <- pmax(0, gfp_im - um_ref)

## -----------------------------
## H reference & IM contamination
## -----------------------------
h_ref <- median(gfp_h_corr, na.rm = TRUE)

if (h_ref > 0) {
  haust_frac <- pmin(gfp_im_corr / h_ref, 1)
} else {
  haust_frac <- rep(0, length(gfp_im_corr))
}

# FORCE authoritative naming
names(haust_frac) <- im_samples

# Safety checks (now should PASS)
stopifnot(
  length(haust_frac) == length(im_idx),
  all(is.finite(haust_frac)),
  all(haust_frac >= 0 & haust_frac <= 1),
  identical(names(haust_frac), im_samples)
)
...
stopifnot(!any(is.na(trap.count.clean[, im_idx])))
