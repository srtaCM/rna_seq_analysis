#!/bin/bash
#SBATCH --job-name=bowtie_GFP1_9
#SBATCH --account=rnaseq-pilotlab 
#SBATCH --output=bowtie_GFP1_9.%j.out
#SBATCH --error=bowtie_GFP1_9.%j.err
...
OUTDIR=$PROJDIR/H_ratio/bowtie/GFP1_9
...
for sample in "${samples[@]}"; do
    ...
    # Count reads mapped to GFP1_9 specifically
    samtools view -c -F 4 $OUTDIR/${sample}.aligned.sorted.bam > $OUTDIR/${sample}.GFP1_9_mapped_count.txt
...
echo -e "sample_id\tGFP1_9_mapped_reads\ttotal_reads\tmapped_pct" > $OUTDIR/GFP1_9_mapping_summary.tsv

for sample in "${samples[@]}"; do
    count_file=$OUTDIR/${sample}.GFP1_9_mapped_count.txt
    ...
    echo -e "${sample}\t${mapped}\t${total}\t${pct}" >> $OUTDIR/GFP1_9_mapping_summary.tsv
done
