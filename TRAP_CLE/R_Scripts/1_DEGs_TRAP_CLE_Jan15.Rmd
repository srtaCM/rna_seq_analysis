...
# Install / update Bioconductor packages (uncomment and edit as needed)
 #BiocManager::install(c( "rprojroot"))
#   "DESeq2",
#   "EnhancedVolcano",
#   "ComplexHeatmap",
#   "reactome.db"
...
```{r setup-output-dirs}
# ===============================
# Setup output directories at PROJECT ROOT (RStudio-friendly)
# ===============================
...
results_dir   <- file.path(project_root, "TRAP_CLE/Results")
...
# Extract contrasts of interest: IM vs UM and H vs UM (UM is the reference) IT vs UT
...
res_IT_UT_df  <- as.data.frame(res_IT_UT)
...
readr::write_csv(res_IT_UT_df,  
                 file.path(tables_dir, "DESeq2_IT_vs_UT.csv"))
...
writexl::write_xlsx(
  list(
    ...
    "H_vs_UM"  = res_H_UM_df,
    "IT_vs_UT" = res_IT_UT_df
  ),
  path = file.path(tables_dir, "DESeq2_results_IM_H_IT.xlsx")
)
...
it_ut_sel <- res_IT_UT_df %>%
  dplyr::select(GeneID, IT_vs_UT_log2FC = log2FoldChange, IT_vs_UT_padj = padj)
...
deseq_all <- dplyr::full_join(im_um_sel, h_um_sel,it_ut_sel, by = "GeneID")
head(deseq_all)
...
cat("✅ Tables exported to Results/Tables:\n")
...
```{r}
# ===============================
# Combined: Shared/Unique + Volcano + Heatmap + Exports
# ===============================
...
# 1. Significant genes (padj < 0.05, |log2FC| > 1)
sig_IM <- rownames(res_IM_UM)[res_IM_UM$padj < 0.05 & abs(res_IM_UM$log2FoldChange) > 1]
sig_H  <- rownames(res_H_UM)[res_H_UM$padj < 0.05 & abs(res_H_UM$log2FoldChange) > 1]

shared    <- intersect(sig_IM, sig_H)
unique_IM <- setdiff(sig_IM, sig_H)
unique_H  <- setdiff(sig_H, sig_IM)

cat("🔬 DE Genes (padj < 0.05, |LFC| > 1):\n")
cat("- Shared (IM & H vs UM):     ", length(shared), "\n")
cat("- IM-unique:                ", length(unique_IM), "\n")
cat("- H-unique (pure haustoria):", length(unique_H), "\n")
...
merged_res <- data.frame(
  ...
  log2FC_IM        = res_IM_UM$log2FoldChange,
  padj_IM          = res_IM_UM$padj,
  log2FC_H         = res_H_UM$log2FoldChange,
  padj_H           = res_H_UM$padj,
  stringsAsFactors = FALSE
) %>%
  mutate(
    category = case_when(
      rownames(.) %in% shared ~ "Shared",
      rownames(.) %in% unique_IM ~ "IM-unique", 
      rownames(.) %in% unique_H ~ "H-unique",
      TRUE ~ "NS"
    ),
    max_padj = pmax(padj_IM, padj_H, na.rm = TRUE)
  )

# Export merged + categories
readr::write_csv(merged_res, file.path(tables_dir, "DE_shared_unique_categories.csv"))

# 3. Concordance scatter
p_concord <- ggplot(merged_res, aes(log2FC_IM, log2FC_H, color = category)) +
  geom_point(alpha = 0.6, size = 1) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black", size = 0.8) +
  scale_color_manual(values = c(Shared = "red3", "IM-unique" = "steelblue", "H-unique" = "darkgreen", NS = "grey70")) +
  labs(title = "Concordance: IM vs UM vs H vs UM", x = "log2FC (IM vs UM)", y = "log2FC (H vs UM)",
       caption = paste("Shared:", length(shared))) +
  theme_minimal(base_size = 12) + 
  theme(legend.position = "bottom", legend.title = element_blank())
print(p_concord)
ggsave(file.path(graphics_dir, "concordance_IM_H_vs_UM.png"), p_concord, width = 8, height = 6, dpi = 300)
...
# 1-3. [Previous sections unchanged: sig genes, merged_res, concordance plot, exports]

# ... (keep sig_IM, sig_H, shared, merged_res, p_concord, ggsave as-is)

# 4. Volcano IM vs UM (FIXED: no 'draws')
p_volc_IM <- EnhancedVolcano(merged_res,
  lab = merged_res$Gene,
  x = 'log2FC_IM', y = 'padj_IM',
  pCutoff = 0.05, FCcutoff = 1,
  pointSize = 1.2, labSize = 2.5,
  title = 'IM vs UM Volcano (TRAP Mesophyll)',
  subtitle = "padj < 0.05 & |log2FC| > 1",
  colAlpha = 0.8,
  legendPosition = 'right',
  col = c("grey40", "blue", "green3", "red"),
  legendLabels = c('NS', 'LFC', 'p-value', 'p-value & LFC'),
  legendLabSize = 10
)
print(p_volc_IM)
ggsave(file.path(graphics_dir, "volcano_IM_vs_UM.png"), p_volc_IM, width = 10, height = 8, dpi = 300)

# 4b. Bonus: Volcano H vs UM 
p_volc_H <- EnhancedVolcano(merged_res,
  lab = merged_res$Gene,
  x = 'log2FC_H', y = 'padj_H',
  pCutoff = 0.05, FCcutoff = 1,
  pointSize = 1.2, labSize = 2.5,
  title = 'H vs UM Volcano (Pure Haustoria)',
  subtitle = "padj < 0.05 & |log2FC| > 1", 
  colAlpha = 0.8,
  legendPosition = 'right',
  col = c("grey40", "blue", "green3", "red")
...
print(p_volc_H)
ggsave(file.path(graphics_dir, "volcano_H_vs_UM.png"), p_volc_H, width = 10, height = 8, dpi = 300)


# 5-6. [Heatmap + top table unchanged]

print(p_volc_IM)
ggsave(file.path(graphics_dir, "volcano_IM_vs_UM.png"), p_volc_IM, width = 10, height = 8, dpi = 300)

# 5. Heatmap: Top SHARED (haustoria signal)
top_shared <- head(shared, min(50, length(shared)))
top_shared <- intersect(top_shared, rownames(dds_UM))  # Safe subset [conversation_history:55]
cat("Heatmap: Top", length(top_shared), "shared genes in dds_UM\n")

if(length(top_shared) > 0) {
  mat <- assay(dds_UM)[top_shared, ]
  annotation_col <- as.data.frame(colData(dds_UM)[, c("Groups", "Condition"), drop = FALSE])
  rownames(annotation_col) <- colnames(mat)
  
  p_heat <- pheatmap(mat, scale = "row",
    annotation_col = annotation_col,
    main = paste("Top", length(top_shared), "Shared DE Genes\n(Haustoria Candidates: IM & H vs UM)"),
    show_rownames = FALSE, show_colnames = FALSE,
    filename = file.path(graphics_dir, "heatmap_top_shared_genes.png"),  # Auto-save
    width = 10, height = 8
  )
} else {
  cat("❌ No shared genes available for heatmap\n")
}

top_shared_df <- merged_res %>%
  filter(Gene %in% shared) %>%
  mutate(avg_lfc = (log2FC_IM + log2FC_H)/2) %>%
  arrange(desc(avg_lfc)) %>%
  head(20)

knitr::kable(top_shared_df[,1:6], digits = 3, caption = "Top 20 Shared DE Genes")  # Nice table in Rmd

readr::write_csv(top_shared_df, file.path(tables_dir, "top20_shared_DE_genes.csv"))
...
# ===============================
# Sample clustering: VST + PCA + Export
# ===============================

# VST (variance stabilizing for PCA/clustering)
vsd <- vst(dds_UM, blind = TRUE)

# PCA data (DESeq2 convenience function)
...
p_pca <- ggplot(pca_data, aes(PC1, PC2, color = Groups, shape = Condition)) +
  geom_point(size = 4, stroke = 1.2, alpha = 0.85) +
  ...
  geom_text_repel(aes(label = name), 
    ...
    max.overlaps = 20, size = 3.2,
    segment.color = "grey60", segment.size = 0.3,
    bg.color = "white", bg.r = 0.1
  ) +
  labs(
    title = "TRAP-seq Sample Clustering (VST PCA)",
    subtitle = "Haustoria enrichment: H vs IM separation",
...
# Export high-res to Graphics/
ggsave(file.path(graphics_dir, "TRAPseq_PCA_vst_samples.png"), 
       p_pca, width = 12, height = 9, dpi = 300, bg = "white")

# Bonus: Sample distance heatmap
sampleDists <- dist(t(assay(vsd)))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- pca_data$name
colnames(sampleDistMatrix) <- NULL
colors <- colorRampPalette(rev(RColorBrewer::brewer.pal(9, "Blues")))(255)
pheatmap(sampleDistMatrix, clustering_distance_rows = sampleDists,
         clustering_distance_cols = sampleDists, col = colors,
         main = "Sample-to-Sample Distances (VST)",
         filename = file.path(graphics_dir, "sample_distances_heatmap.png"))
