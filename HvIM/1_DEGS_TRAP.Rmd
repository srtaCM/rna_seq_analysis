...
# Read featureCounts (skip header, geneid as rownames)
featcount <- read_table ("./Data_Repository/featcount.txt", col_names = TRUE, skip = 1)

# Clean featcounts: geneid → rownames, drop metadata cols 1-6
...
stopifnot(ncol(read.count) == length(sample_names)) # Check
colnames(read.count) <- sample_names #commit naming system
...
if(length(top_DE_genes) > 0) {
  ...
  # Sample annotation
  ann_col <- data.frame(Groups = sample_info_UM$Groups)
  ...
  p_deg <- pheatmap(deg_mat,
    ...
    show_rownames = ifelse(nrow(deg_mat) < 30, TRUE, FALSE),
    show_colnames = FALSE,
...
# 1. Check what keytypes org.At.tair.db accepts
keytypes(org.At.tair.db)

# 2. See example valid TAIR keys
head(keys(org.At.tair.db, keytype = "TAIR"))

# 3. Check your TAIR format
head(deg_H$TAIR, 10)  # Print first 10
sample_tair <- deg_H$TAIR[1:5]
sample_tair

pad <- 0.05
lfc_cut <- 1

# 1. DE tables for each contrast (padj + |LFC|)
deg_H <- as_tibble(res_H_UM, rownames = "TAIR") %>%
  filter(padj < pad, abs(log2FoldChange) >= lfc_cut)

deg_IM <- as_tibble(res_IM_UM, rownames = "TAIR") %>%
  filter(padj < pad, abs(log2FoldChange) >= lfc_cut)

# 2. Add symbols (optional; TAIR is enough for enrichGO)
deg_H <- deg_H %>%
  mutate(symbol = mapIds(org.At.tair.db,
                         keys = TAIR,
                         column = "SYMBOL",
                         keytype = "TAIR",
                         multiVals = "first"))
