...
# QC summaries
sig_H_UM <- sum(res_H_UM_df$padj < pad & abs(res_H_UM_df$log2FoldChange) >= lfc_thresh)
sig_IM_UM <- sum(res_IM_UM_df$padj < pad & abs(res_IM_UM_df$log2FoldChange) >= lfc_thresh)

cat("\n🔬 DEGs (padj <", pad, "|LFC| ≥", lfc_thresh, "):\n")
...
sum(lfc_H_UM_df$padj < pad & abs(lfc_H_UM_df$log2FoldChange) >= lfc_thresh)

sum(lfc_IM_UM_df$padj < pad & abs(lfc_IM_UM_df$log2FoldChange) >= lfc_thresh)
```
...
deg_H_up   <- deg_H %>% filter(direction == "H_UP")
deg_H_down <- deg_H %>% filter(direction == "H_DOWN")
...
# Label direction
ego_H_up@result$Direction <- "H-UP"
ego_H_down@result$Direction <- "H-DOWN"

# Combine GO results
ego_combined <- bind_rows(
  ego_H_up@result %>% select(Description, GeneRatio, p.adjust, Direction),
  ego_H_down@result %>% select(Description, GeneRatio, p.adjust, Direction)
)

# Convert GeneRatio to numeric
ego_combined <- ego_combined %>%
...
# Select top GO terms from each direction
top_GO <- ego_combined %>%
  group_by(Direction) %>%
...
top_GO$Description <- factor(
  top_GO$Description,
  levels = rev(unique(top_GO$Description))
)

# Plot
p_heatmap <- ggplot(top_GO, aes(
  x = Direction,
  y = Description,
  fill = logP
)) +
  ...
  geom_text(aes(label = round(GeneRatio_num, 2)), size = 3) +
  labs(
    x = "",
    y = "GO Biological Process",
    fill = "-log10(adj p-value)",
    title = "GO BP Enrichment Heatmap: H-UP vs H-DOWN Haustoria Genes"
  ) +
  theme_minimal() +
  theme(
...
# Save
# Save figure
ggsave(file.path(graphics_dir, "GO_H_UP_vs_DOWN_heatmap.png"),
...
```{r}
## KEGG
entrez <- bitr(green_H_up$id, from="TAIR", to="ENTREZID", OrgDb=org.At.tair.db) %>% distinct(ENTREZID)
...
transporter_genes <- transporter_list$TAIR

green_H_up_transporters <- green_H_up %>%
...
transporters_fourway <- dplyr::bind_rows(
  green_H_up_transporters  %>% mutate(group = "H_up"),
  green_H_down_transporters %>% mutate(group = "H_down"),
  red_IM_up_transporters %>% mutate(group = "IM_up"),
  red_IM_down_transporters %>% mutate(group = "IM_down")
)
  
transporters_fourway %>%
  select(id, group, logFC_x, logFC_y, padj_x, padj_y)
  
transporters_fourway %>%
  dplyr::count(group)
  
transporters_fourway <- transporters_fourway %>%
  left_join(transporter_list, by = c("id" = "TAIR"))
dplyr::count(transporters_fourway, group, Family)
```
```{r}
# =============================================
# R Script: H-up Transporter Candidate Table & Plot
# =============================================
...
library(readr)
library(forcats)
# Prepare transporter list

# transporter_list should have columns:
# tair (ATG ID), family (transporter family)
# transporters_fourway has id (ATG ID) and DE statistics
...
H_up_transporters <- green_H_up %>%        # green quadrant, H-specific UP
  filter(id %in% transporter_list$id) %>% # intersect with transporter genes
...
write_csv(H_up_transporters,
          file.path(graphics_dir, "H_up_transporter_candidates.csv"))
```
...
# Only plot top 15 transporters by fold change
top_n <- 10
plot_df <- H_up_transporters %>%
  slice(1:top_n) %>%
  mutate(id = fct_reorder(id, logFC_x)) # reorder bars

p_transporter <- ggplot(plot_df, aes(x = id, y = logFC_x, fill = Family)) +
  geom_col(width = 0.7) +
  geom_text(aes(label = FC_label), vjust = -0.3, size = 3) +
  scale_fill_brewer(palette = "Set2", name = "Transporter Family") +
  labs(
...
# Save the plot
ggsave(file.path(graphics_dir, "H_up_transporter_barplot.png"),
       p_transporter,
       width = 10,
       height = 6,
       dpi = 300)
